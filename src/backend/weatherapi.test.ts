...
it("should convert API request", async () => {
  ...
  await fetchWeatherData(axios, WEATHER_API_URL, "0.0", "0.0", "Test Location, US");
});
...
it("throws error when response is not 200", async () => {
  ...
  // this will throw an error if it fails, which will mark the test as a failure
  await expect(fetchWeatherData(axios, WEATHER_API_URL, "0.0", "0.0", "Test Location, US")).rejects.toThrow();
});
