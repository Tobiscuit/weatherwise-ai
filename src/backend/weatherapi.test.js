...
it("should convert API request", async () => {
    ...
    await (0, weatherapi_1.fetchWeatherData)(axios_1.default, WEATHER_API_URL, "0.0", "0.0", "Test Location, US");
});
it("throws error when response is not 200", async () => {
    ...
    // this will throw an error if it fails, which will mark the test as a failure
    await expect((0, weatherapi_1.fetchWeatherData)(axios_1.default, WEATHER_API_URL, "0.0", "0.0", "Test Location, US")).rejects.toThrow();
});
...
it("gets low temp", () => {
    ...
    assert_1.strict.equal(lowTemp, 23.3);
});
it("gets high temp", () => {
    const response = weatherapi_1.currentWeatherApiResponseSchema.parse(SAMPLE_API_RESPONSE);
    const currentWeather = new weatherapi_1.CurrentWeather(response);
    const highTemp = currentWeather.highTemp();
    assert_1.strict.equal(highTemp, 33.5);
});
it("gets condition", () => {
    const response = weatherapi_1.currentWeatherApiResponseSchema.parse(SAMPLE_API_RESPONSE);
    const currentWeather = new weatherapi_1.CurrentWeather(response);
    const condition = currentWeather.condition();
    assert_1.strict.equal(condition, "Clear sky");
});
